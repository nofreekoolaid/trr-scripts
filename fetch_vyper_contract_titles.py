...
ETHERSCAN_API_URLS = {"eth": "https://api.etherscan.io/api", "arb": "https://api.arbiscan.io/api"}
...
def get_api_key(network):
    if network == "eth":
        ...
        if not api_key:
            raise ValueError(
                "Error: ETHERSCAN_API_KEY is not set. Please set it as an environment variable."
            )
        return api_key
        ...
        if not api_key:
            raise ValueError(
                "Error: ARBISCAN_API_KEY is not set. Please set it as an environment variable."
            )
        return api_key
...
def main(json_file, network):
    ...
    for name, address in contracts.items():
        ...
        results[name] = {"address": address, "contract_title": contract_title}
