...
class InteractionFilter:
    def __init__(self, config: Dict):
        ...
        self.allowed_call_types = set(config.get('allowed_call_types', ["CALL", "DELEGATECALL", "STATICCALL"]))
    ...
    def filter_interactions(self, interactions: List[str], source_addr: str, tx_data: Dict = None) -> List[str]:
        ...
        blacklist = {Web3.to_checksum_address(addr) for addr in self.config.get('blacklist_contracts', [])}
    ...
    def _should_keep_interaction(self, target_addr: str, source_addr: str, tx_data: Dict) -> bool:
        ...
        if tx_data and 'logs' in tx_data:
            for log in tx_data['logs']:
                print("Log entry:")
                print(log)
                if (Web3.to_checksum_address(log.get('address', '')) == target_addr and 
                    self.has_allowed_event_signature(log)):
                    print("Allowed event signature found, keeping interaction.")
                    return True
