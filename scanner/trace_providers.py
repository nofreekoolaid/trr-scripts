...
class TraceProvider(ABC):
    ...
    @abstractmethod
    def extract_direct_calls(self, trace: Dict, source_addr: str, allowed_call_types: Set[str] = None) -> Set[str]:
...
class TenderlyTraceProvider(TraceProvider):
    ...
    def extract_direct_calls(self, trace: Dict, source_addr: str, allowed_call_types: Set[str] = None) -> Set[str]:
        ...
        def _extract_from_node(node, caller=None):
            ...
            current_caller = Web3.to_checksum_address(node.get("from", "")) if node.get("from") else None
...
class GethTraceProvider(TraceProvider):
    ...
    def extract_direct_calls(self, trace: Dict, source_addr: str, allowed_call_types: Set[str] = None) -> Set[str]:
        ...
        return TenderlyTraceProvider.extract_direct_calls(self, trace, source_addr, allowed_call_types)
    ...
    def extract_direct_calls(self, trace: Dict, source_addr: str, allowed_call_types: Set[str] = None) -> Set[str]:
        ...
        if trace and "trace" in trace:
            for trace_entry in trace["trace"]:
                ...
                if (from_checksum == source_checksum and 
                    to_checksum and 
                    call_type in allowed_call_types):
